...
class TFSViT(Algorithm):
    ...
    def __init__(self, input_shape, num_classes, num_domains, hparams):
        super(TFSViT, self).__init__(input_shape, num_classes, num_domains,
                                        hparams)

        # create model
        backbone = self.hparams['backbone']
        self.network = return_tfsvit_backbone_network(backbone, num_classes, hparams)
...
class ATFSViT(Algorithm):

    def __init__(self, input_shape, num_classes, num_domains, hparams):
        super(ATFSViT, self).__init__(input_shape, num_classes, num_domains,
                                         hparams)

        # create model
        backbone = self.hparams['backbone']
        self.network = return_atfsvit_backbone_network(backbone, num_classes, hparams)
...
class SDViT_TFSViT(Algorithm):

    def __init__(self, input_shape, num_classes, num_domains, hparams):
        super(SDViT_TFSViT, self).__init__(input_shape, num_classes, num_domains,
                                                     hparams)

        print("---- init SDViT_TFSViT algorithm...")
...
class SDViT(Algorithm):
    """
    Implementation of Self-Distilled Vision Transformer for
    Domain Generalization, Sultana et al. 2022.
    """

    def __init__(self, input_shape, num_classes, num_domains, hparams):
        super(SDViT, self).__init__(input_shape, num_classes, num_domains,
                                        hparams)
        self.alpha_rb_loss = self.hparams['RB_loss_weight']
        self.alpha_KL_temp = self.hparams['KL_Div_Temperature']
...
class Distribution_shift(Algorithm):
    """
    Empirical Risk Minimization (ERM)
    """
    ...
    def __init__(self, input_shape, num_classes, num_domains, hparams):
        super(ERM, self).__init__(input_shape, num_classes, num_domains,
                                  hparams)
        self.featurizer = networks.small_CNN(input_shape)
        self.classifier = networks.Classifier(
            self.featurizer.n_outputs,
            2,
            self.hparams['nonlinear_classifier'])
    ...
    def pixmix(self, x, x_mixingpic):
        mixings = utils.mixings

        if np.random.random() < 0.5:
            mixed = self.augment_input(x)
        else:
            mixed = x
        ...
        for _ in range(np.random.randint(4 + 1)):
            ...
            if np.random.random() < 0.5:
                aug_image_copy = self.augment_input(x)
            else:
                aug_image_copy = x_mixingpic
            ...
            aug_image_copy = aug_image_copy.to(device)
        ...
        return normalize(mixed).to(device)
    ...
    def augment_input(self, x):
        ...
        out_list = torch.cat([torch.unsqueeze(trans1(op(transform(im), 1)), 0) for im in x])
        return out_list
...
def return_tfsvit_backbone_network(network_name, num_classes, hparams):
    if network_name == "DeitSmall":
        network = torch.hub.load(
            ...
            pretrained=True, source='local', num_layers=hparams['num_layers'],
            d_rate=hparams['d_rate'], alpha=hparams['alpha'],
            )
        network.head = nn.Linear(384, num_classes)
        return network
    elif network_name == "T2T14":
        network = tfsvit_t2t_vit_t_14(num_layers=hparams['num_layers'],
                                              d_rate=hparams['d_rate'], alpha=hparams['alpha'])
...
def return_atfsvit_backbone_network(network_name, num_classes, hparams):
    ...
    if network_name == "DeitSmall":
        ...
        network = atfsvit_t2t_vit_t_14(num_layers=hparams['num_layers'],
                                                            d_rate=hparams['d_rate'], alpha=hparams['alpha'])
